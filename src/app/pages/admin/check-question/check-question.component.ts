...
export class CheckQuestionComponent implements OnInit {
  ...
  UpdateTableData(reset: boolean = false): void {
    ...
    this.loading = true;
    this.http_client
      .get<MyServerResponse>(this.base_url + "question/all")
      .subscribe(
        response => {
          this.question_info_list = response.data;
          if (this.question_info_list == null) this.question_info_list = [];
          for (let i = 0; i < this.question_info_list.length; i++) {
            this.question_info_list[i].options = JSON.parse(
              this.question_info_list[i].options
            );
            this.question_info_list[i].answer = JSON.parse(
              this.question_info_list[i].answer
            );
  ...
  GetAnswer(index: number): string {
    let current_question = (this.page_index - 1) * this.page_size + index;
    if (this.question_info_list[current_question].type == "subjective")
      return "鼠标悬浮查看详情";
    var answers_info = "";
  ...
  CheckQuestionInfo(index: number): void {
    if (index == -1) {
      ...
      this.current_select_question =
        (this.page_index - 1) * this.page_size + index;
      this.edit_question_id = this.question_info_list[
        this.current_select_question
      ].id;
      this.edit_question_type = this.question_info_list[
        this.current_select_question
      ].type;
      this.edit_question_content = this.question_info_list[
        this.current_select_question
      ].content;
      this.edit_question_description = this.question_info_list[
        this.current_select_question
      ].description;
      this.edit_question_options = this.question_info_list[
        this.current_select_question
      ].options;
      this.edit_question_answer = this.question_info_list[
        this.current_select_question
      ].answer;
      this.edit_question_knowledge = this.question_info_list[
        this.current_select_question
      ].knowledge;
    }
  ...
  DeleteKnowledge() {
    ...
    this.http_client
      .delete<MyServerResponse>(this.base_url + "keypoint/del", httpOptions)
      .subscribe(
        response => {
  ...
  UpdateKnowledgeInfo(is_open: boolean = true) {
    ...
    this.http_client
      .get<MyServerResponse>(this.base_url + "keypoint/all")
      .subscribe(
        response => {
          this.all_knowledge_info = response.data;
          this.edit_knowledge_dic = {};
          this.edit_knowledge_flag_dic = {};
          if (this.all_knowledge_info == null) this.all_knowledge_info = [];
          for (let i = 0; i < this.all_knowledge_info.length; i++) {
            this.edit_knowledge_dic[
              this.all_knowledge_info[i].key
            ] = this.all_knowledge_info[i].title;
            this.edit_knowledge_flag_dic[
              this.all_knowledge_info[i].key
            ] = false;
          }
  ...
  EditQuestionInfo(): void {
    ...
    this.http_client
      .post<MyServerResponse>(
        this.base_url + "question/single",
        question_edit_info
      )
      .subscribe(
        response => {
          if (response.status != 200) {
            ...
            this.message.create(
              "success",
              "试题 " + this.edit_question_content + " 编辑成功"
            );
            this.edit_question_info_loading = false;
            this.drawer_visible = false;
            this.UpdateTableData();
  ...
  DeleteQuestion(index: number): void {
    ...
    this.http_client
      .delete<MyServerResponse>(this.base_url + "question/del", httpOptions)
      .subscribe(
        response => {
          if (response.status != 200) {
            ...
            this.message.create(
              "success",
              "试题 " +
                this.question_info_list[this.current_select_question].content +
                " 删除成功"
            );
            this.UpdateTableData();
          }
  ...
  handleInputConfirm(): void {
    if (
      this.inputValue &&
      this.user_group_tags.indexOf(this.inputValue) === -1
    ) {
  ...
  DownloadTemplate(): void {
    ...
    this.http_client
      .get(this.base_url + "question/template", {
  ...
  limit() {
    if (this.edit_question_answer[0].content.length > 20000)
      this.edit_question_answer[0].content = this.edit_question_answer[0].content.substring(
        0,
        20000
      );
  }
  ...
  TypeSelectChanged(event: string) {
    if (event != "subjective") this.edit_question_answer = [];
    else this.edit_question_answer = [{ id: 0, content: "" }];
  }

  EditKnowledgeInfo(knowledge_id: string, knowledge_name: string = null) {
    if (knowledge_id != "") this.edit_knowledge_name = knowledge_name;
    ...
    this.http_client
      .post<MyServerResponse>(this.base_url + "keypoint/single", knowledge_info)
      .subscribe(
        response => {
          if (response.status != 200) {
  ...
  SearchQuestionContent() {
    this.question_info_list = this.filter_sort_service.GetSearchedArray(
      this.question_info_list,
      this.question_content_search_val,
      "content"
    );
  }
  ...
  UpdateFilteredData() {
    ...
    this.question_info_list = this.filter_sort_service.GetFilteredArray(
      this.question_info_list,
      this.question_type_filter_val,
      "type"
    );
  }

  Validate(): boolean {
    if (this.edit_question_type == null || this.edit_question_type == "")
      return false;
    if (this.edit_question_type != "subjective") {
      if (
        this.edit_question_content == null ||
        this.edit_question_content == ""
      )
        return false;
      if (
        this.edit_question_options == null ||
        this.edit_question_options.length == 0
      )
        return false;
      for (let option of this.edit_question_options)
        if (option["content"] == "") return false;
      if (
        this.edit_question_answer == null ||
        this.edit_question_answer.length == 0
      )
        return false;
    } else {
      if (
        ...
        return false;
      if (
        this.edit_question_answer[0]["content"] == null ||
        this.edit_question_answer[0]["content"] == ""
      )
        return false;
    }
    return true;
