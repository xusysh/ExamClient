...
export class GeneratePaperComponent implements OnInit {
  ...
  GetQuestionHead(question: object): string {
    switch (question["type"]) {
      case "single":
        return "[" + "单选题" + "] " + question["content"];
      case "multi":
        return "[" + "不定项选择题" + "] " + question["content"];
      case "judge":
        return "[" + "判断题" + "] " + question["content"];
      case "subjective":
        return "[" + "简答题" + "] " + question["content"];
      default:
        return "";
    }
  }

  @ViewChild("content_canvas", { static: false })
  content_canvas_element_view: ElementRef;
  public canvas_height: number = 0;
  public elem_height_str: string =
    Math.ceil(document.documentElement.clientHeight * 0.68).toString() + "px";
  ...
  constructor(
    ...
    this.http_client
      .post<MyServerResponse>(this.base_url + "paper/single", paper_req)
      .subscribe(
        response => {
          ...
          for (let i = 0; i < this.paper_info.categoryList.length; i++) {
            ...
            let category_question_list = this.paper_info.categoryList[i]
              .questionList;
  ...
  GetKnowledgeStr(question: QuestionInfo): string {
    if (question.knowledge.length == 0 || question.knowledge == null)
      return "无";
    let knowledge_str = "";
    ...
    this.http_client
      ...
      .subscribe(
        response => {
          ...
          if (
            this.all_filtered_questions == null ||
            this.all_filtered_questions == undefined
          )
            this.all_filtered_questions = [];
          this.paper_question_scores = new Array<string>(
            this.all_filtered_questions.length
          );
          this.selected_values = new Array<string>(
            this.all_filtered_questions.length
          );
          for (let i = 0; i < this.all_filtered_questions.length; i++) {
            this.all_filtered_questions[i].options = JSON.parse(
              this.all_filtered_questions[i].options
            );
            this.all_filtered_questions[i].answer = JSON.parse(
              this.all_filtered_questions[i].answer
            );
          }
  ...
  GetFilteredQuestions() {
    ...
    this.http_client
      .post<MyServerResponse>(this.base_url + "question/filter", filter)
      .subscribe(
        response => {
          this.all_filtered_questions = response.data;
          if (
  ...
  CategorySelectChanged(index: number) {
    ...
    if (this.category_to_questions.has(this.selected_values[index])) {
      this.category_to_questions
        .get(this.selected_values[index])
        .push(paper_question_info);
    } else {
      this.category_to_questions.set(this.selected_values[index], [
        paper_question_info
      ]);
    }
  }
  ...
  EditCateGoryDone(index: number) {
    if (
      this.categorys.indexOf(this.edit_category_name) != -1 &&
    ...
    if (
      !this.category_to_questions.has(this.edit_category_name) &&
      this.category_to_questions.has(this.categorys[index])
    ) {
      this.category_to_questions.set(
        this.edit_category_name,
        this.category_to_questions.get(this.categorys[index])
      );
  ...
  ShowSubmitConfirm() {
    this.confirm_modal = this.modal.confirm({
      ...
      nzOnOk: () => {
        ...
        for (let i = 0; i < this.categorys.length; i++) {
          let question_list: Array<PaperQuestionInfo> = this.category_to_questions.get(
            this.categorys[i]
          );
        ...
        this.http_client
          .post<MyServerResponse>(this.base_url + "paper/new", new_paper_info)
          .subscribe(
            response => {
              if (response.status == 200) {
  ...
  QuesTypeFilterChange(type: string, checked: any) {
    if (checked) {
      ...
      if (index != -1) this.ques_type_filter.splice(index, 1);
    }
    this.GetFilteredQuestions();
  }
